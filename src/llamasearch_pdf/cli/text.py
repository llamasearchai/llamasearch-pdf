...
from ..core import TextExtractor, extract_text
...
def batch_extract_command(args):
    ...
    with Progress(
        ...
        overall_task = progress.add_task(
            f"Processing {len(pdf_files)} files", total=len(pdf_files)
        )
    ...
    console.print(
        f"[green]Text extraction complete. Results saved to: {output_dir}[/green]"
    )
    return 0
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    extract_parser = subparsers.add_parser(
        "extract", help="Extract text from a single PDF"
    )
    extract_parser.add_argument("input", help="Input PDF file")
    extract_parser.add_argument(
        "--output",
        "-o",
        help="Output text file (default: input file with .txt extension)",
    )
    extract_parser.add_argument(
        "--pages",
        "-p",
        help="Pages to extract (comma-separated, ranges allowed: e.g., '1-3,5,7-9')",
    )
    extract_parser.add_argument(
        "--flow-text",
        "-f",
    ...
    extract_parser.add_argument(
        "--stdout", "-s", action="store_true", help="Print extracted text to stdout"
    )
    extract_parser.set_defaults(func=extract_text_command)

    # Batch text extraction
    batch_parser = subparsers.add_parser(
        "batch", help="Extract text from multiple PDFs"
    )
    batch_parser.add_argument("input_dir", help="Input directory containing PDFs")
    batch_parser.add_argument(
        "output_dir", help="Output directory for extracted text files"
    )
    batch_parser.add_argument(
        "--recursive",
        "-r",
