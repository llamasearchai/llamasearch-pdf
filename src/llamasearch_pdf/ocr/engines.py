        ...
        try:
            ...
            if isinstance(image, (str, Path)):
                return pytesseract.image_to_string(
                    str(image), lang=lang, config=config, timeout=timeout
                )
            else:
                return pytesseract.image_to_string(
    ...
    def process_image(self, image: Union[str, Path, Image.Image], **kwargs) -> str:
        ...
        try:
            ...
            result = self.model.chat(
                self.tokenizer, img_path, ocr_type=ocr_type.lower()
            )
    ...
    def process_pdf(
        ...
        **kwargs,
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                images = convert_from_path(
                    str(pdf_path), dpi=dpi, output_folder=temp_dir, fmt="png"
                )
            except Exception as e:
                logger.error(f"Error converting PDF to images: {e}")
                return None
            ...
            for idx, image in enumerate(images, start=1):
                ...
                try:
                    ocr_result = self.process_image(
                        temp_image_path, ocr_type=kwargs.get("ocr_type", self.ocr_type)
                    )
