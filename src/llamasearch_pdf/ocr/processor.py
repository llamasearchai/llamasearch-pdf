...
class OCRProcessor:
    ...
    def __init__(
        ...
        if engine is None:
            ...
            if self.engine is None:
                raise ValueError(
                    "No OCR engines available. Please install Tesseract OCR, OCRmyPDF, or transformers."
                )
            ...
            if engine.lower() not in engines:
                ...
                raise ValueError(
                    f"OCR engine '{engine}' not available. Available engines: {available}"
                )
    ...
    def batch_process(
        ...
        **kwargs,
        ...
        if self.num_workers > 1:
            if self.verbose:
                logger.info(
                    f"Processing {len(files)} files with {self.num_workers} workers"
                )
            ...
            with ThreadPoolExecutor(max_workers=self.num_workers) as executor:
                ...
                for file_path in files:
                    ...
                    if output_dir:
                        if output_format.lower() == "pdf":
                            output_path = (
                                output_dir / f"{file_path.stem}_ocr{file_path.suffix}"
                            )
                        else:
                    ...
                    if file_path.suffix.lower() in [".pdf"]:
                        ...
                        future = executor.submit(
                            self.ocr_image, file_path, output_path, **kwargs
                        )
            ...
            for file_path in files:
                ...
                try:
                    ...
                    if file_path.suffix.lower() in [".pdf"]:
                        result = self.ocr_pdf(
                            file_path, output_path, output_format, **kwargs
                        )
                    else:
                        # Assume image
                        if output_format.lower() == "pdf":
                        ...
                        result = self.ocr_image(file_path, output_path, **kwargs)
    ...
    def process_directory(
        ...
        **kwargs,
        ...
        if not files:
            logger.warning(
                f"No files with extensions {file_extensions} found in {directory}"
            )
            return {}
        ...
        # Process the files
        return self.batch_process(files, output_dir, output_format, **kwargs)
    ...
    def get_available_engines() -> Dict[str, str]:
        ...
        engines = get_available_engines()
        return {
            name: engine.__doc__.strip().split("\n")[0]
