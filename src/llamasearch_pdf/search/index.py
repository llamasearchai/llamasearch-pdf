...
class SearchIndex:
    ...
    def search(
        ...
        filtered_scores = [
            (doc_info, score)
            for doc_info, score in scores.items()
            if score >= min_score
        ]
        ...
        for (doc_id, page_num), score in top_results:
            ...
            snippet = self._generate_snippet(
                doc_id, page_num, query_tokens, snippet_size
            )
    ...
    def _score_documents(self, query_tokens: List[str]) -> Dict[Tuple[str, int], float]:
        ...
        for token in set(query_tokens):
            ...
            for doc_id, page_num, _ in self.inverted_index[token]:
                ...
                doc_tf = sum(
                    1
                    for d, p, _ in self.inverted_index[token]
    ...
    def _generate_snippet(
        self, doc_id: str, page_num: int, query_tokens: List[str], size: int = 100
    ) -> str:
        """
...
def create_index(
    ...
    return SearchIndex(
        case_sensitive=case_sensitive, stopwords=stopwords_set, index_path=index_path
    )
