    ...
    def compress_image(
        ...
        try:
            ...
            with Image.open(input_path) as img:
                ...
                if img.mode in ("RGBA", "LA") and output_path.suffix.lower() in (
                    ".jpg",
                    ".jpeg",
                ):
                    ...
                    img = background
                elif img.mode == "P" and output_path.suffix.lower() not in (
                    ".png",
                    ".gif",
                ):
                    img = img.convert("RGB")
                ...
                if output_path.suffix.lower() in (".jpg", ".jpeg"):
                    ...
                    img.save(
                        output_path, "WEBP", quality=self.webp_quality, method=6
                    )  # Higher value = better compression but slower
                else:
    ...
    def process_directory(
        ...
        if progress:
            task_id = progress.add_task(
                "[cyan]Compressing images...", total=len(image_files)
            )
        ...
        for img_path in image_files:
            ...
            if output_folder:
                rel_path = (
                    img_path.relative_to(folder_path) if recursive else img_path.name
                )
...
def compress_images_cli(
    ...
    with Progress(
        ...
        success_count, total_count = compressor.process_directory(
            input_dir, output_dir, recursive=recursive, progress=progress
        )
    ...
    if output_dir:
        ...
        console.print(
            "Compressed images saved in the original directory with '-compressed' suffix."
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Compress and optimize images for PDF inclusion"
    )
    parser.add_argument("input_dir", help="Directory containing images to compress")
    parser.add_argument(
        "-o", "--output-dir", help="Directory where compressed images should be saved"
    )
    parser.add_argument(
        "-q", "--quality", type=int, default=85, help="JPEG quality (0-100)"
    )
    parser.add_argument(
