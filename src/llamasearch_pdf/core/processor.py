...
class PDFProcessor:
    ...
    def __init__(
        ...
        self.output_base_dir = (
            Path(output_dir) if output_dir else Path("llamasearch_output")
        )
        self.timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        self.max_workers = max_workers or (os.cpu_count() or 4)
        self.supported_image_formats = (
            supported_image_formats or self.SUPPORTED_IMAGE_FORMATS
        )
    ...
    def find_files(
        ...
        task_id = None
        if progress:
            task_id = progress.add_task("[cyan]Scanning directory...", total=None)
    ...
    def batch_convert_images(
        ...
        if progress:
            task_id = progress.add_task(
                "[cyan]Converting images to PDF...", total=len(image_files)
            )
        ...
        def convert_image_with_progress(image_path: Path) -> Optional[Path]:
            pdf_path = self.convert_image_to_pdf(
                image_path,
                output_path=(
                    output_dir / f"{image_path.stem}.pdf" if output_dir else None
                ),
            )
            if progress and task_id is not None:
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            futures = [
                executor.submit(convert_image_with_progress, img) for img in image_files
            ]
    ...
    def merge_pdfs(
        ...
        if progress:
            task_id = progress.add_task(
                "[cyan]Merging PDF files...", total=len(pdf_files)
            )
    ...
    def process_directory(
        ...
        with Progress(
            ...
            self.console.print(
                f"Found [cyan]{len(pdf_files)}[/cyan] PDFs and [cyan]{len(image_files)}[/cyan] images"
            )
            ...
            if image_files:
                ...
                converted_pdfs = self.batch_convert_images(
                    image_files, progress=progress
                )
            ...
            if merge and (pdf_files or converted_pdfs):
                ...
                if self.merge_pdfs(all_pdfs, merged_path, progress=progress):
                    self.console.print(
                        f"[green]Successfully created merged PDF: {merged_path}"
                    )
                    ...
                    if optimize:
                        ...
                        if self.optimize_pdf(
                            merged_path, optimized_path, progress=progress
                        ):
                            self.console.print(
                                f"[green]Successfully optimized PDF: {optimized_path}"
                            )
                ...
                self.console.print(
                    f"[green]Processed {len(pdf_files) + len(converted_pdfs)} PDF files"
                )
    ...
    def optimize_pdf(
        ...
        task_id = None
        if progress:
            task_id = progress.add_task("[cyan]Optimizing PDF...", total=None)
