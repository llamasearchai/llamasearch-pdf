...
class TextExtractor:
    ...
    def extract_text_from_pdf(
        ...
        if progress:
            task_id = progress.add_task(
                f"Extracting text from {pdf_path.name}", total=None
            )
        ...
        try:
            ...
            if progress and task_id is not None:
                progress.update(
                    task_id,
                    total=num_pages if page_numbers is None else len(page_numbers),
                )

            # Determine page range
            pages_to_extract = (
                page_numbers if page_numbers is not None else range(num_pages)
            )
            ...
            for i in pages_to_extract:
                if i < 0 or i >= num_pages:
                    logger.warning(
                        f"Page {i} is out of range for PDF {pdf_path} ({num_pages} pages)"
                    )
                    continue
                ...
                try:
                    ...
                    if not text or text.isspace():
                        if self.fallback_to_ocr:
                            logger.info(
                                f"No text found in page {i} of {pdf_path}. Falling back to OCR."
                            )
    ...
    def extract_text_from_pdfs(
        self, pdf_paths: List[Union[str, Path]], progress: Optional[Progress] = None
        ...
        task_id = None
        if progress:
            task_id = progress.add_task("Processing PDF files", total=len(pdf_paths))

        for pdf_path in pdf_paths:
            try:
                pdf_path = Path(pdf_path)
                results[str(pdf_path)] = self.extract_text_from_pdf(
