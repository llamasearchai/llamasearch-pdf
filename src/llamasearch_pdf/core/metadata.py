        ...
        if "/Metadata" in reader.trailer["/Root"]:
            try:
                ...
                creator_match = re.search(
                    r"<dc:creator>.*?<rdf:Seq>.*?<rdf:li>(.*?)</rdf:li>",
                    xmp_data.decode("utf-8", errors="ignore"),
                    re.DOTALL,
                )
                if creator_match:
    ...
    def print_metadata_summary(self, metadata: Dict[str, Any]) -> None:
        ...
        if "page_sizes" in metadata and metadata["page_sizes"]:
            ...
            if len(set(tuple(p.items()) for p in metadata["page_sizes"])) == 1:
                # All pages same size
                print(
                    f"Page Size: {first_page['width']} × {first_page['height']} {first_page['unit']}"
                )
            else:
                # Mixed page sizes
...
def extract_metadata(pdf_path: Union[str, Path], strict: bool = True) -> Dict[str, Any]:
    """
...
def create_basic_metadata(
    ...
    return manager.create_metadata(
        title=title, author=author, creator="LlamaSearch PDF Toolkit"
    )
