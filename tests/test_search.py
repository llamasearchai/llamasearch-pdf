...
from llamasearch_pdf.search import SearchIndex, SearchResult, create_index, search_pdfs
...
class TestSearchIndex(unittest.TestCase):
    ...
    def create_test_pdf(self):
        ...
        try:
            ...
            c.drawString(
                100, 700, "This is a sample document created for search testing."
            )
            c.drawString(
                100,
                650,
                "It contains specific words and phrases for search evaluation.",
            )
    ...
    def test_case_sensitivity(self):
        ...
        if "PYTHON" not in str(
            test_pdf
        ):  # Only if the test PDF doesn't contain "PYTHON"
            self.assertNotEqual(
                len(results_sensitive_lower), len(results_sensitive_upper)
            )
