...
def create_sample_pdf(output_path):
    ...
    try:
        ...
        c.drawString(
            100,
            680,
            "Quantum computing is a rapidly emerging technology that leverages",
        )
        c.drawString(
            100, 660, "the principles of quantum mechanics to process information."
        )
        c.drawString(
            100, 640, "Unlike classical computers that use bits, quantum computers"
        )
        c.drawString(
            100,
            620,
        ...
        c.drawString(
            100, 630, "These applications could revolutionize their respective fields."
        )
        c.showPage()
        ...
        c.drawString(
            100, 670, "Applications include machine translation, sentiment analysis,"
        )
...
def demonstrate_search_index():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(
            f"  Index loaded successfully with {loaded_index.get_stats()['document_count']} documents"
        )
        ...
        # Example searches
        queries = ["quantum computing", "artificial intelligence", "natural language"]
...
def demonstrate_quick_search():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Create some sample PDFs
        pdf_files = []
        for i, topic in enumerate(["quantum_computing", "artificial_intelligence"]):
            pdf_path = temp_dir_path / f"{topic}.pdf"
            create_sample_pdf(pdf_path)
...
if __name__ == "__main__":
    ...
    print(
        "This example demonstrates how to use the search capabilities of LlamaSearch PDF."
    )
