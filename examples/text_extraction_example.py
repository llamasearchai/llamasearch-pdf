...
def example_simple_extraction(pdf_path, output_path=None):
    ...
    result = extract_text(
        pdf_path, output_path=output_path, preserve_layout=True, verbose=True
    )
    ...
    if output_path:
        ...
        if result:
            ...
            text_snippet = (
                result[first_page][:500] + "..."
                if len(result[first_page]) > 500
                else result[first_page]
            )
...
def example_advanced_extraction(pdf_path, output_dir=None):
    ...
    if matching_pages:
        console.print(
            f"Found keyword '{keyword}' on pages: {[p+1 for p in matching_pages]}"
        )
    else:
...
def example_comparison(pdf_path):
    ...
    console.print(
        f"Native extraction: {len(native_text)} pages, {native_char_count} characters"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Text extraction examples for llamasearch-pdf"
    )
    parser.add_argument("--input", "-i", required=True, help="Input PDF file")
    parser.add_argument(
        "--output", "-o", help="Output directory for extraction results"
    )
    parser.add_argument("--all", "-a", action="store_true", help="Run all examples")
    parser.add_argument(
        "--simple", "-s", action="store_true", help="Run simple extraction example"
    )
    parser.add_argument(
        "--advanced", "-d", action="store_true", help="Run advanced extraction example"
    )
    parser.add_argument(
        "--compare", "-c", action="store_true", help="Run extraction comparison example"
    )
