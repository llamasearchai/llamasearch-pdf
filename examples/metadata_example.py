...
def example_extract_metadata(pdf_path):
    ...
    for field in standard_fields:
        ...
        value = metadata.get(
            field,
            metadata.get(
                field.capitalize(), metadata.get(f"/{field.capitalize()}", None)
            ),
        )
        if value:
    ...
    if "page_sizes" in metadata and metadata["page_sizes"]:
        ...
        if len(set(tuple(p.items()) for p in metadata["page_sizes"])) == 1:
            # All pages same size
            size_str = (
                f"{first_page['width']} × {first_page['height']} {first_page['unit']}"
            )
...
def example_update_metadata(pdf_path, output_path):
    ...
    new_metadata = create_basic_metadata(
        title="Updated PDF Document", author="Nik Jois"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Metadata examples for llamasearch-pdf"
    )
    ...
    parser.add_argument(
        "--extract", "-e", action="store_true", help="Run metadata extraction example"
    )
    parser.add_argument(
        "--update", "-u", action="store_true", help="Run metadata update example"
    )
    parser.add_argument(
        "--text",
        "-t",
        action="store_true",
        help="Run text-based metadata extraction example",
    )
    parser.add_argument(
        "--export", "-x", action="store_true", help="Export metadata to JSON"
    )
